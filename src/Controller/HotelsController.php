...
class HotelsController extends AppController
{
    ...
    public function index()
    {
        ...
        if ($this->request->is(['post'])) {
            ...
            $city = $state = $country = "";
            $address = str_replace(" ", "+", $this->request->data['destination']);
            $url = "http://maps.google.com/maps/api/geocode/json?address=$address&sensor=false";
            $result = file_get_contents("$url");
            $json = json_decode($result);

                foreach($json->results[0]->address_components as $addressPart) {

                    if((in_array('locality', $addressPart->types)) && (in_array('political', $addressPart->types)))
                        $city = str_replace(' ','%20',$addressPart->long_name);
                    else if((in_array('administrative_area_level_1', $addressPart->types)) && (in_array('political', $addressPart->types)))
                        $state = str_replace(' ','%20',$addressPart->long_name);
                    else if((in_array('country', $addressPart->types)) && (in_array('political', $addressPart->types)))
                        $country = $addressPart->short_name;
            }

//            echo $city.' '.$country.' '.$state;die;
            ...
            $result = 150;
            $xml = "%3CHotelListRequest%3E%20%3Ccity%3E" . $city . "%3C/city%3E%20%3CstateProvinceCode%3E". $state . "%3C/stateProvinceCode%3E%0A%20%3CcountryCode%3E" . $country . "%3C/countryCode%3E%0A%20%3CarrivalDate%3E" . $arrival . "%3C/arrivalDate%3E%0A%20%3CdepartureDate%3E" . $departure . "%3C/departureDate%3E%0A%20%3CRoomGroup%3E%0A%20%3CRoom%3E%0A%20%3CnumberOfAdults%3E" . $adults . "%3C/numberOfAdults%3E%0A%20%3CnumberOfChildren%3E" . $kids . "%3C/numberOfChildren%3E%0A%20%3C/Room%3E%0A%20%3C/RoomGroup%3E%0A%20%3CnumberOfResults%3E" . $result . "%3C/numberOfResults%3E%0A%20%3C/HotelListRequest%3E";
...
////                         debug($hotelPrices); die;
    ...
    public function available(){
        ...
        $data= $this->Ean->getCurlData($path,$xml);
            ...
            $path = "ean-services/rs/hotel/v3/roomImages";
            $xml ="%3CRoomImageRequest%3E%0A%20%20%20%20%3ChotelId%3E".$this->request->data['id']."%3C%2FhotelId%3E%0A%3C%2FRoomImageRequest%3E";
            $image = $this->Ean->getCurlData($path,$xml);


        $this->set('image', json_decode($image, true));
    ...
    public function booking($supplier_type= null,$room_key = null, $hotel_id=null){
        ...
        if($dataArray['HotelRoomAvailabilityResponse']['@size']<=1){
            ...
            $this->set('hotelData',['customerSessionId'=>$dataArray['HotelRoomAvailabilityResponse']['customerSessionId'],'hotelId'=>$hotel_id]);
...
//            pr($dataArray); die;
    ...
    public function getCity()
    {
	 ...
	 public function rezdy(){
        // 8fd549d9b96547be8dc64b1a09c1a5c5

        $url = "https://api.rezdy.com/v1/products?apiKey=8fd549d9b96547be8dc64b1a09c1a5c5";
        $ch = curl_init();
